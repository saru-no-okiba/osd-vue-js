    ...
    <div id="head">
      <div id="search_area">
        <form role="search">
          ...
          <span ref="msg" id="msg">{{ msg }}</span>
        </form>
      </div><!--search_area end-->

      <div id="page_area">
        <input id="page_box" v-bind:value="selectedPage" v-on:input="selectedPage = $event.target.value" v-on:keyup.enter="toSelectedPage" />
        <span>／</span>
        <span>{{totalPage}}</span>
          ...
          <div v-for="(item, index) in filteredData" v-bind:key="index" v-bind:id="'txt'+(index+1)">
            <div v-for="(str, idx) in item" v-bind:key="idx">
              <span
                ...
                v-on:click="toPage(index,str.$.X,str.$.Y,str.$.WIDTH,str.$.HEIGHT,idx)"
                v-on:keyup.enter="toPage(index,str.$.X,str.$.Y,str.$.WIDTH,str.$.HEIGHT,idx)"
                role="button"
                tabIndex="0"
                >
                【{{index+1}}】{{ str.$.STRING }}
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        xml2js.parseString(response.data, (err, result) => {
          if (err) {
            ...
            this.allData = result.OCRDATASET.PAGE.flat(0);
            console.log("データ取得完了");
            this.msg = "";

            // 総ページ数の設定
            this.totalPage = result.OCRDATASET.PAGE.length;

            // tileSource の作成
            this.tileSourcesArray = this.allData.map((data) => ({
              type: "image",
              url: "./data/" + data.$.IMAGENAME,
            }));
          }
        });
      } catch (error) {
    ...
    isSearch:function(w){
      ...
      let kw = this.searchKw.trim(); // 検索ワード,前後の空白を削除
      this.msg = "";
      ...
      // キーワード検索
      const isMatch = (line) =>
        kwsArray.every(keyword => _.includes(this.laceAllByConvertTable(line.$.STRING), keyword));
    ...
    //ページ遷移
    toPage: function (index, x, y, w, h, idx) {
      this.viewer.goToPage(index);//該当ページへ

      // 該当画像幅取得
      const iw = this.allData[index].$.WIDTH;
    ...
    getPage:function(){
      ...
      this.currentImg=this.allData[idx].$.IMAGENAME;//表示画像名取得
    ...
    paramsCheck: function () {
      ...
      // クエリパラメータの取得とチェック
      let pagecode = urlParams.has('pagecode') ? parseInt(urlParams.get('pagecode')) : null;
      let p = urlParams.has('p') ? parseInt(urlParams.get('p')) : null;
    ...
    //文字の正規化
    laceAllByConvertTable:function(searchWords){
      var res="";
      res = searchWords;
